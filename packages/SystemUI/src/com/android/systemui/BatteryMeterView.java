...
public class BatteryMeterView extends View implements DemoMode {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mMeterMode.compareTo(BatteryMeterMode.BATTERY_METER_CIRCLE) == 0) {
    ...
    public void setMode(BatteryMeterMode mode) {
        if (mMeterMode == null || mMeterMode.compareTo(mode) != 0) {
            mMeterMode = mode;
            if (mode.compareTo(BatteryMeterMode.BATTERY_METER_GONE) == 0) {
                ...
                if (mMeterMode.compareTo(BatteryMeterMode.BATTERY_METER_ICON_PORTRAIT) == 0 ||
                        mMeterMode.compareTo(BatteryMeterMode.BATTERY_METER_ICON_LANDSCAPE) == 0) {
    ...
    protected class NormalBatteryMeterDrawable implements BatteryMeterDrawable {
        ...
        protected final boolean mInverted;
        ...
        public NormalBatteryMeterDrawable(Context ctx, boolean inverted) {
            super();
            mInverted = inverted;
            mDisposed = false;
        ...
        public void onDraw(Canvas c) {
            ...
            mButtonHeight = (int) ((mInverted ? width : height) * 0.12f);
            ...
            if (mInverted) {
                mButtonFrame.set(
                        width - (mButtonHeight + 5) - mFrame.left /*cover frame border of intersecting area*/,
            ...
            if (tracker.plugged) {
                // draw the bolt
                final float bl = (int)(mFrame.left + mFrame.width() / (mInverted ? 9f : 4.5f));
                final float bt = (int)(mFrame.top + mFrame.height() / (mInverted ? 4.5f : 6f));
                final float br = (int)(mFrame.right - mFrame.width() / (mInverted ? 6f : 7f));
                final float bb = (int)(mFrame.bottom - mFrame.height() / (mInverted ? 7f : 10f));
                ...
                final float x = (mInverted ? mHeight : mWidth) * 0.5f;
                final float y = mInverted
                ...
                final float full = mInverted ? 0.60f : 0.40f;
                final float nofull = mInverted ? 0.75f : 0.5f;
                final float single = mInverted ? 0.86f : 0.75f;
                mTextPaint.setTextSize(height *
                        (SINGLE_DIGIT_PERCENT ? single
                                : (tracker.level == 100 ? full : nofull)));
                mTextHeight = -mTextPaint.getFontMetrics().ascent;
                mTextWidth = mTextPaint.measureText(String.valueOf(tracker.level));

                final String str = String.valueOf(SINGLE_DIGIT_PERCENT ? (level/10) : level);
                final float x = mInverted ? mWidth - mTextWidth : mWidth * 0.5f;
        ...
        private float[] loadBoltPoints(Resources res) {
            final int[] pts = res.getIntArray(getBoldPointsArrayResource());
            int maxX = 0, maxY = 0;
    ...
    protected class CircleBatteryMeterDrawable implements BatteryMeterDrawable {
        ...
        private void drawCircle(Canvas canvas, int status, int level, int animOffset,
            ...
            if (unknownStatus) {
                ...
                // draw the bolt
                final float bl = (int)(drawRect.left + drawRect.width() / 3.5f);
                final float bt = (int)(drawRect.top + drawRect.height() / 4f);
                final float br = (int)(drawRect.right - drawRect.width() / 5f);
