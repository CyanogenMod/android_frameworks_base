...
public class CompassTile extends QuickSettingsTile implements
    ...
    private final static int COMPASS_TILE_UPDATE_INTERVAL = 200;
    private final static float DEFAULT_IMAGE_ROTATION = 90f;
    ...
    private synchronized void updateTile() {
        if (mActive && mPrepared) {
            ...
            mHandler.removeCallbacks(mCompassTileUpdater);
        }
    }

    Runnable mCompassTileUpdater = new Runnable() {
        @Override
        public void run() {
            updateCompassTile();
            mHandler.postDelayed(mCompassTileUpdater, COMPASS_TILE_UPDATE_INTERVAL);
        }
    };
    ...
    private void updateCompassTile() {
        // Exit when we don't have a orientation change or the tile
        // was deactivated while the function got called
        if (Math.round(mDegree) == Math.round(mCurrentDegree) || !mActive || !mPrepared) {
            return;
        }
        ...
        mDegree = -(mDegree + DEFAULT_IMAGE_ROTATION);

        // Create a rotation animation
        RotateAnimation rotateAnimation = new RotateAnimation(mCurrentDegree, mDegree,
                Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);

        // Set animation properties
        rotateAnimation.setDuration((int) (Math.abs(mCurrentDegree - mDegree) % 360) * 2);
