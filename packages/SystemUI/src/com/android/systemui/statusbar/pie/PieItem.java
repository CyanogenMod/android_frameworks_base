...
public class PieItem extends PieLayout.PieDrawable {
    ...
    public void onClickCall() {
        if (mOnClickListener != null)
            mOnClickListener.OnClick(this, name);
    }

    public PieItem(Context context, PieLayout parent, int width, String name, View view) {
        mView = view;
        mPieLayout = parent;
        this.name = name;
        this.width = width;
    ...
    public void show(boolean show) {
        flags = show ? (flags | PieLayout.PieDrawable.VISIBLE) : (flags & ~PieLayout.PieDrawable.VISIBLE);
    }

    public void setSelected(boolean selected) {
        mPieLayout.postInvalidate();
        flags = selected ? (flags | PieLayout.PieDrawable.SELECTED) : (flags & ~PieLayout.PieDrawable.SELECTED);
    }
    ...
    public void prepare(Position position, float scale) {
        ...
        if (mView != null) {
            ...
            final float radius = (h * 1.3f < mOuter - mInner) ?
                    mInner + (mOuter - mInner) * 2.0f/3.0f : (mInner + mOuter) / 2.0f;

            int l = (int)(Math.cos(Math.toRadians(mStart + mSweep / 2)) * radius * scale);
            int t = (int)(Math.sin(Math.toRadians(mStart + mSweep / 2)) * radius * scale);
    ...
    public void draw(Canvas canvas, Position position) {
        canvas.drawPath(mPath, (flags & PieDrawable.SELECTED) != 0 ? mSelectedPaint : mBackgroundPaint);
    ...
    private Path getOutline(float scale) {
        ...
        float alphaOuter = (float)Math.toDegrees(Math.asin((mInner + mOuter) * Math.sin(Math.toRadians(mGap / 2.0f)) / (mOuter * 2.0f)));
        float alphaInner = (float)Math.toDegrees(Math.asin((mInner + mOuter) * Math.sin(Math.toRadians(mGap / 2.0f)) / (mInner * 2.0f)));
