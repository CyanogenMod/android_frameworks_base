...
public class PieSysInfo extends PieSliceContainer implements ValueAnimator.AnimatorUpdateListener{
    ...
    public PieSysInfo(Context context, PieLayout parent, PieController controller, int initialFlags) {
    ...
    public void prepare(Position position, float scale) {
        ...
        mStaleData = true;
        mClockText = getSimpleTime();
        mAmPmText = getAmPm();

        mClockPaint.setAlpha(0);
        mAmPmPaint.setAlpha(0);
        mInfoPaint.setAlpha(0);

        final Resources res = mContext.getResources();
        final RectF innerBB = new RectF(-mInner * scale, -mInner * scale, mInner * scale, mInner * scale);
        ...
        mAmPmPaint.setTextSize((mOuter - mInner) * scale / 3.0f);
        ...
        for (int i = 0; i < mInfoPath.length; i++)
            mInfoPath[i] = updatePath(mInfoPath[i], (mInner + textsize * 1.2f * i) * scale, 272, mStart + mSweep - 272);
    }
    ...
    public void draw(Canvas canvas, Position position) {
        ...
        canvas.drawTextOnPath(mAmPmText, mClockPath, lastPos, 0, mAmPmPaint);

        if (mController.hasTelephony())
            canvas.drawTextOnPath(mNetworkProvider, mInfoPath[3], 0, 0, mInfoPaint);
        canvas.drawTextOnPath(mSimpleDate, mInfoPath[2], 0, 0, mInfoPaint);
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        int alpha = (int)(0xff * animation.getAnimatedFraction());
    ...
    private void updateData() {
        ...
        mSimpleDate = getSimpleDate();
        mWifiSsid = getWifiSsid();

        mBatteryLevelReadable = mController.getBatteryLevelReadable();
    }
    ...
    private String getSimpleDate() {
        SimpleDateFormat sdf = new SimpleDateFormat(mContext.getString(R.string.pie_date_format));
