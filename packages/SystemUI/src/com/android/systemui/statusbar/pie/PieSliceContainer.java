...
public class PieSliceContainer extends PieLayout.PieSlice {
            ...
            if (position != Position.TOP) {
                for (PieItem item : mItems)
                    if ((item.flags & viewMask) == viewMask) {
                        item.setGeometry(mStart + deltaSweep * width, item.width * deltaSweep, mInner, mOuter);
                        item.setGap(deltaSweep * gapMinder);

                        if (PieLayout.DEBUG)
                            Slog.d(PieLayout.TAG, "Layout " + item.name + " : (" + (mStart + deltaSweep * width) + "," + (item.width * deltaSweep) + ")");
    ...
    public PieItem findItem(String name) {
        for (PieItem item : mItems)
