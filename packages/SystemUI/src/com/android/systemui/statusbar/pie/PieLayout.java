...
public class PieLayout extends FrameLayout implements View.OnTouchListener {
    ...
    private ValueAnimator mBackgroundAnimator = ValueAnimator.ofFloat(0.0f, 1.0f).setDuration(TIME_FADEIN);
    private List<ValueAnimator.AnimatorUpdateListener> mAnimationListenerCache = new ArrayList<ValueAnimator.AnimatorUpdateListener>();
    ...
    private ValueAnimator.AnimatorUpdateListener mUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            if (DEBUG)
                Slog.d(TAG, "Animation update " + animation.getAnimatedFraction() + " on: " + Thread.currentThread().getName());
    ...
    public abstract static class PieDrawable {
        ...
        public boolean hit(float alpha, int radius) {
            return (alpha > mStart) && (alpha < mStart + mSweep) && (radius > mInner) && (radius < mOuter);
        }

        // Display on all positions
        public final static int DISPLAY_ALL = Position.LEFT.FLAG | Position.BOTTOM.FLAG | Position.RIGHT.FLAG | Position.TOP.FLAG;
        // Display on all except the TOP position
        public final static int DISPLAY_NOT_AT_TOP = Position.LEFT.FLAG | Position.BOTTOM.FLAG | Position.RIGHT.FLAG;
    ...
    public abstract static class PieSlice extends PieDrawable {
        ...
        /**
         * The slice will be considerer as important - {@link PieLayout} will try to keep these slices on screen, when
         * placing the pie control.
         *
        ...
        public float estimateWidth() {
            return (float)(Math.abs(Math.cos(Math.toRadians(mStart + mSweep)) - Math.cos(Math.toRadians(mStart))) * mOuter);
        }
        ...
        public boolean interact(float x, float y) {
            ...
            if (distanceSqr - mSnapRadiusSqr < mSnapThresholdSqr) {
                ...
                if (distanceSqr < mSnapRadiusSqr) {
                    if (DEBUG)
                        Slog.d(TAG, "Snap point " + position.name() + " activated with (" + x + ","+ y + ")");
                    mActivity = 1.0f;
                    return true;
                } else {
    ...
    private void getDimensions() {
        ...
        mSnapThreshold = Math.min(res.getDisplayMetrics().heightPixels, res.getDisplayMetrics().widthPixels) * 0.3f;
    ...
    private void getColors() {
        ...
        mSnapPaint.setColor(res.getColor(android.R.color.holo_blue_light));
        ...
        mSnapActivePaint.setColor(res.getColor(android.R.color.holo_blue_light));
    ...
    public void onDraw(Canvas canvas) {
        if (mActive) {
            ...
            if (dActivateStart != 0)
                Slog.d(TAG,  "First draw within " + (SystemClock.uptimeMillis() - dActivateStart) + " ms");
            dActivateStart = 0;


            mBackgroundPaint.setAlpha((int)(mBackgroundFraction * Color.alpha(COLOR_BACKGROUND)));
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (mActive) {
            if (MotionEvent.ACTION_DOWN == action) {
            ...
            } else if (MotionEvent.ACTION_MOVE == action || MotionEvent.ACTION_UP == action) {
                mActiveSnap = null;
                for (int i = 0; i < mSnapPoints.length; i++)
                    if (mSnapPoints[i] != null) {
                ...
                float alpha = (float)((distance > 1.0f)?Math.toDegrees(Math.atan2(y - mCenter.y, x - mCenter.x)):0.0f);
                if (alpha < 0.0f) alpha = 360 + alpha;
                ...
                for (PieDrawable drawable : mDrawableCache) {
                    ...
                    if (tmp != null) newItem = tmp;
                }
                if (newItem != mActiveItem) {
                    if (newItem != null) newItem.setSelected(true);
                    if (mActiveItem != null) mActiveItem.setSelected(false);
                }
                mActiveItem = newItem;
            }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (DEBUG)
            Slog.d(TAG, "onLayout: " + changed + " (" + left + "," + top + "." + right + "," + bottom + ")");

        long start = SystemClock.uptimeMillis();
        if (dActivateStart != 0)
            Slog.d(TAG,  "Layout within " + (start - dActivateStart) + " ms");
        ...
        if (changed || mPosition != mLayoutDoneForPosition) {
            ...
            for (PieSlice slice : mSlices) {
                ...
                if ((slice.flags & viewMask) == viewMask) {
                    ...
                    // This is not nice, but it will help to keep the PieSlice abstract class more clutter free.
        ...
        if (mPosition == Position.LEFT || mPosition == Position.RIGHT) {
            ...
            if (estimatedWidth * 1.3f > getHeight()) {
                ...
                mCenter.y = (int)(Math.min(Math.max(estimatedWidth / 2, mCenter.y), getHeight() - estimatedWidth / 2));
            }
        } else { /* Position.BOTTOM | Position.TOP */
            if (estimatedWidth * 1.3f > getWidth()) {
                mCenter.x = getWidth() / 2;
            } else {
                mCenter.x = (int)(Math.min(Math.max(estimatedWidth / 2, mCenter.x), getWidth() - estimatedWidth / 2));
            }
    ...
    public void activate(Point center, Position position) {
        ...
        dActivateStart = SystemClock.uptimeMillis();
        ...
        Slog.d(TAG, "activate finished within " + (SystemClock.uptimeMillis() - dActivateStart) + " ms");
    }
