...
public class PieController implements BaseStatusBar.NavigationBarCallback,
    ...
    private static class KeyEventHolder {
        KeyEventHolder(long downTime, int keyCode) {
    ...
    private class H extends Handler {
        public void handleMessage(Message m) {
            ...
            final KeyEventHolder keh = (KeyEventHolder) m.obj;
            switch (m.what) {
                case MSG_INJECT_KEY_DOWN:
                    inputManager.injectInputEvent(new KeyEvent(keh.downTime, eventTime,
                            KeyEvent.ACTION_DOWN, keh.keyCode, 0, 0,
                            KeyCharacterMap.VIRTUAL_KEYBOARD, 0,
                            KeyEvent.FLAG_FROM_SYSTEM | KeyEvent.FLAG_VIRTUAL_HARD_KEY,
                            InputDevice.SOURCE_KEYBOARD),
                            InputManager.INJECT_INPUT_EVENT_MODE_ASYNC);
                    mPieContainer.playSoundEffect(SoundEffectConstants.CLICK);
                    mHandler.sendMessageDelayed(Message.obtain(mHandler, MSG_INJECT_KEY_UP, keh), 20);
                    break;
                case MSG_INJECT_KEY_UP:
                    inputManager.injectInputEvent(new KeyEvent(keh.downTime, eventTime,
                            KeyEvent.ACTION_UP, keh.keyCode, 0, 0,
                            KeyCharacterMap.VIRTUAL_KEYBOARD, 0,
                            KeyEvent.FLAG_FROM_SYSTEM | KeyEvent.FLAG_VIRTUAL_HARD_KEY,
                            InputDevice.SOURCE_KEYBOARD),
                            InputManager.INJECT_INPUT_EVENT_MODE_ASYNC);
    ...
    private void injectKeyDelayed(int keyCode, long when) {
        // only intercept events, that didn't submit the ACTION_DOWN event
        mHandler.removeMessages(MSG_INJECT_KEY_DOWN);

        KeyEventHolder keh = new KeyEventHolder(when, keyCode);
        mHandler.sendMessageDelayed(Message.obtain(mHandler, MSG_INJECT_KEY_DOWN, keh), 10);
    }
    ...
    private void setupNavigationItems() {
        ...
        for (int i = 0; i < buttons.length; i++) {
            if (buttons[i] != NavigationButtons.EMPTY) {
                ...
                boolean isSmall = NavigationButtons.BUTTON_IS_SMALL[i];
