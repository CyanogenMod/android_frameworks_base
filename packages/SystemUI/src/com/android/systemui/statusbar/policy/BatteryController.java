...
public class BatteryController extends BroadcastReceiver {
    ...
    /**
     * The action that should be registered to get battery events
     *
     * @return The action
     */
    protected String getBatteryAction() {
    ...
    protected boolean isBatteryStatusUnknown() {
        // Main battery never has an unknown status
        return false;
    }

    protected boolean isBatteryStatusCharging() {
        // aosp ignore the status flag for this widget, so we are going to ignore too.
        // Assume that if is plugged then is on charge
        return mBatteryPlugged;
    }

    protected boolean isBatteryPlugged() {
        // this widget always shows the battery status. aosp uses EXTRA_PLUGGED for
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(getBatteryAction())) {
    ...
    private void updateSettings() {
        ...
        if (mBatteryStyle == BATTERY_STYLE_NORMAL || mBatteryStyle == BATTERY_STYLE_PERCENT) {
            // register receivers
            if (!mReceiverRegistered) {
                IntentFilter filter = new IntentFilter();
                filter.addAction(getBatteryAction());
