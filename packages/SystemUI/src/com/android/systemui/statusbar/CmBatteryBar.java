...
public class CmBatteryBar extends ProgressBar {
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            boolean powerConnected = Intent.ACTION_POWER_CONNECTED.equals(action);
            if (powerConnected || Intent.ACTION_BATTERY_CHANGED.equals(action)) {
                if (!powerConnected) {
                    mBatteryLevel = intent.getIntExtra("level", 0);
                }
                mBatteryCharging = intent.getIntExtra("status", 0) == BatteryManager.BATTERY_STATUS_CHARGING;
                if ((powerConnected || mBatteryCharging) && mBatteryLevel < 100) {
                    boolean oldBatteryCharging = mBatteryCharging;
                    if (!oldBatteryCharging || powerConnected) {
                        startTimer();
                    }
    ...
    public synchronized void setProgress(int progress) {
        ...
        if (d instanceof LayerDrawable) {
            ...
            if (progressBar != null) {
                progressBar.mutate();
                if (mShowLowBattery && progress <= 15) {
                    progressBar.setColorFilter(Color.RED, PorterDuff.Mode.SRC);
                } else if (mShowLowBattery && progress <= 30) {
