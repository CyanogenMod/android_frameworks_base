...
public class ClockController {
    ...
    private void updateSettings(boolean selfUpdate) {
        ContentResolver resolver = mContext.getContentResolver();
        int amPmStyle = CMSettings.System.getIntForUser(resolver,
                CMSettings.System.STATUS_BAR_AM_PM, Clock.AM_PM_STYLE_GONE,
                UserHandle.USER_CURRENT);
        boolean amPmStyleChanged = false;
        if (amPmStyle != mAmPmStyle) {
            mAmPmStyle = amPmStyle;
            amPmStyleChanged = true;
        }
        ...
        updateActiveClock();
        if (amPmStyleChanged && !selfUpdate) {
            // Both center clock + AM/PM visibility require
            // dimensions to be calculated for overflow of icons
            mController.computeMaxWidth();
        }
    }
