...
public class NavbarEditor implements OnTouchListener {
    ...
    private void adjustPadding() {
        ...
        ViewGroup navButtons = (ViewGroup) mParent.findViewById(R.id.nav_buttons);
        //Hide the extra padding on hybrid devices
        for (int c = 0; c < navButtons.getChildCount(); c++) {
            View v = navButtons.getChildAt(c);
            if (v != null && !(v instanceof KeyButtonView) && !(v instanceof LinearLayout)) {
                v.setVisibility(editMode ? View.GONE : View.INVISIBLE);
            }
        }
