...
public class NavbarEditor implements OnTouchListener {
    ...
    protected void saveKeys() {
        ...
        ButtonInfo[] buttons = new ButtonInfo[NavigationButtons.BUTTON_COUNT];
        List<Integer> idMap = (List<Integer>) mIds.clone();
        if (mVertical) Collections.reverse(idMap);
        for (int i = 0; i < NavigationButtons.BUTTON_COUNT; i++) {
    ...
    protected void updateKeys() {
        ...
        for (ButtonInfo bi : buttons) {
            ...
            boolean isSmallButton = NavigationButtons.BUTTON_IS_SMALL[cc];
