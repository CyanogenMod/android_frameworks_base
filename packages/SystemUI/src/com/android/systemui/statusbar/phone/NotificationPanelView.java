...
public class NotificationPanelView extends PanelView implements
    ...
    private static final float QUICK_PULLDOWN_PERCENTAGE = 0.2f;
    ...
    private boolean shouldQuickSettingsIntercept(float x, float y, float yDiff, boolean useHeader) {
        ...
        float region = (w * QUICK_PULLDOWN_PERCENTAGE);
        boolean showQsOverride = false;

        switch (mOneFingerQuickSettingsIntercept) {
            case 1: // Right side pulldown
                shoqQsOverride = isLayoutRtl() ? (x < region) : (w - region < x);
                break;
            case 2: // Left side pulldown
                shoqQsOverride = isLayoutRtl() ? (w - region < x) : (x < region);
                break;
        }
    ...
    private class SettingsObserver extends UserContentObserver {
        ...
        protected void observe() {
            ...
            resolver.registerContentObserver(Settings.Secure.getUriFor(
                    Settings.Secure.QS_QUICK_PULLDOWN), false, this, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.Secure.getUriFor(
        ...
        public void update() {
            ContentResolver resolver = mContext.getContentResolver();
            mOneFingerQuickSettingsIntercept = Settings.Secure.getIntForUser(resolver,
