...
public class NotificationPanelView extends PanelView {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (PhoneStatusBar.SETTINGS_DRAG_SHORTCUT && mStatusBar.mHasFlipSettings) {
            ...
            if (mSwipeTriggered || swipeFlipJustFinished) {
                // Make up a new event, ignoring horizontal movement (X axis)
                MotionEvent original = event;
                event = MotionEvent.obtain(original.getDownTime(), original.getEventTime(),
                    original.getAction(), original.getX(0), mGestureStartY,
                    original.getPressure(0), original.getSize(0), original.getMetaState(),
                    original.getXPrecision(), original.getXPrecision(), original.getDeviceId(),
