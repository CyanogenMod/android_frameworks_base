...
public class AvdBatteryMeterView extends BatteryMeterView implements DemoMode,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mMeterMode == BatteryMeterMode.BATTERY_METER_CIRCLE) {
            width = getResources().getDimensionPixelSize(R.dimen.battery_meter_circle_width);
            height = getResources().getDimensionPixelSize(R.dimen.battery_meter_circle_height);
    ...
    protected class CircleBatteryMeterDrawable extends StopMotionVectorDrawable
        ...
        public CircleBatteryMeterDrawable(Resources res) {
            super((AnimatedVectorDrawable) getContext().getDrawable(R.drawable.ic_battery_circle));
            mBoltDrawable = getContext().getDrawable(R.drawable.ic_battery_bolt);
            mDisposed = false;
        ...
        private void drawCircle(Canvas canvas, BatteryTracker tracker,
            ...
            if (unknownStatus) {
                ...
                mBoltDrawable.setTint(getColorForLevel(level));
        ...
        private void initSizeBasedStuff() {
            ...
            mTextPaint.setTextSize(diameter / 2f);
            mWarningTextPaint.setTextSize(diameter / 2f);

            int pLeft = getPaddingLeft();
            Rect iconBounds = new Rect(pLeft, 0, pLeft + diameter, diameter);
            getDrawable().setBounds(iconBounds);
            mBoltDrawable.setBounds(iconBounds);
            mRectLeft = new RectF(iconBounds);

            // calculate Y position for text
            Rect bounds = new Rect();
            mTextPaint.getTextBounds("99", 0, "99".length(), bounds);
            mTextX = diameter / 2.0f + getPaddingLeft();
            // the +1dp at end of formula balances out rounding issues.works out on all resolutions
            mTextY = diameter / 2.0f + (bounds.bottom - bounds.top) / 2.0f
