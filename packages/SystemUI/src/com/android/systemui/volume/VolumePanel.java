    ...
    protected void onShowVolumeChanged(int streamType, int flags, MediaController controller) {
        ...
        if (sc != null) {
            ...
            if (isNotificationOrRing(streamType)) {
                ...
                if (!mVolumeLinkNotification && mExtendedPanelExpanded) {
                    final StreamControl notification =
                            mStreamControls.get(AudioManager.STREAM_NOTIFICATION);
                    if (notification != null) {
                        updateSlider(notification, false);
                    }
                }
            }
        }
