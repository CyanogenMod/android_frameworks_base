...
public class SpamMessageProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        case MESSAGE:
            ...
            String notificationPkgId = NotificationTable.TABLE_NAME + "." + NotificationTable.PACKAGE_ID;
            qb.appendWhere(pkgId + "=" + notificationPkgId);
            SQLiteDatabase db = mDbHelper.getReadableDatabase();
            Cursor ret = qb.query(db, new String[]{NotificationTable.TABLE_NAME + ".*"}, selection, selectionArgs, null, null, null);
            ret.moveToFirst();
            return ret;
        case MESSAGE_FOR_ID:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        case MESSAGE:
            ...
            if (packageId != -1) {
                ...
                values.put(NotificationTable.NORMALIZED_TEXT, SpamFilter.getNormalizedContent(msgText));
    ...
    private void notifyChange() {
        getContext().getContentResolver().notifyChange(SpamFilter.sNotificationUri, null);
    }

    private void removePackageIfNecessary(int packageId) {
        long numEntries = DatabaseUtils.queryNumEntries(mDbHelper.getReadableDatabase(), NotificationTable.TABLE_NAME,
                NotificationTable.PACKAGE_ID + "=?", new String[]{String.valueOf(packageId)});
        if (numEntries == 0) {
            mDbHelper.getWritableDatabase().delete(PackageTable.TABLE_NAME, PackageTable.ID + "=?", new String[]{String.valueOf(packageId)});
        }
    }
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        case MESSAGE_UPDATE_COUNT:
            String updateQuery = "UPDATE %s SET %s=%s,%s=%s+1 WHERE %s=%s";
            String formattedQuery = String.format(updateQuery, NotificationTable.TABLE_NAME,
                    NotificationTable.LAST_BLOCKED, System.currentTimeMillis(), NotificationTable.COUNT,
                    NotificationTable.COUNT, NotificationTable.ID, uri.getLastPathSegment());
            System.out.println("Here3 " + formattedQuery);
