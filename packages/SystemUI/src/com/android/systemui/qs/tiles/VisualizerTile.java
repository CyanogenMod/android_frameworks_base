...
public class VisualizerTile extends QSTile<QSTile.State>
    ...
    public void onActiveSessionsChanged(@Nullable List<MediaController> controllers) {
        if (controllers != null) {
            ArrayList<MediaSession.Token> tokensToRemove = new ArrayList<>(mCallbacks.keySet());
            for (MediaController controller : controllers) {
                ...
                tokensToRemove.remove(controller.getSessionToken());
            }
            for (MediaSession.Token token : tokensToRemove) {
                CallbackInfo info = mCallbacks.remove(token);
                info.destroy();
            }
        }
    }
