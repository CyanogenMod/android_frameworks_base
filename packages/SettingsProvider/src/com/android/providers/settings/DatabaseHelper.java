    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
       ...
       if (upgradeVersion == 51) {
        ...
        if (upgradeVersion < 118) {
            // On CM11 we've added new defaults on these for certain devices,
            // and added a default for animator duration. Load them if the user
            // hasn't set them.
            db.beginTransaction();
            SQLiteStatement stmt = null;
            try {
                stmt = db.compileStatement("INSERT OR IGNORE INTO " +
                        TABLE_SYSTEM +"(name,value) VALUES(?,?);");
                loadDefaultAnimationSettings(stmt);
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
