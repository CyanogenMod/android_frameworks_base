...
public class AppDock extends LinearLayout implements View.OnTouchListener, SlidingUpPanelLayout.PanelSlideListener {
    ...
    private class FetchApps extends AsyncTask<Void, Void, ArrayList<ComponentName>> {
        ...
        protected ArrayList<ComponentName> doInBackground(Void... params) {
            return SmartPackageStatsUtils.getRelevantComponentsAWeekAgo(getContext(), 8);
        }
    ...
    public void startDipAnimation() {
        ...
        mFadeIn.setFloatValues(1.25f, 10f);
    ...
    protected void onAttachedToWindow() {
        ...
        getContext().getContentResolver().registerContentObserver(
                SmartPackageStatsContracts.AUTHORITY_URI, true, mObserver);
    }

    private void init(Context context) {
        setWillNotDraw(false);
        populatePackages(true);
    ...
    private void populatePackages(boolean noDelay) {
        removeCallbacks(mPopulatePackagesRunnable);
        postDelayed(mPopulatePackagesRunnable, noDelay ? 0 : 500);
    }
    ...
    public void launchApp() {
        ...
        try {
            mKeyguardView.launchAcitivity(i);
    ...
    private void populateView(ImageView v, ComponentName activityInfo) {
        ...
        try {
            ...
            Palette pallete = Palette.generate(((BitmapDrawable) icon).getBitmap(), 50);

            int highestPopulation = 0;
            for (PaletteItem color : pallete.getPallete()) {
                System.out.println(String.format("#%06X", (0xFFFFFF & color.getRgb())) + " " + activityInfo.toString() + " " + color.toString());
            ...
            if (pallete.getVibrantColor() != null) {
                bestColor = pallete.getVibrantColor().getRgb();
            }
...
boolean mHiding;
    ...
    KeyguardHostView mKeyguardView;
    public void setSecurityCallback(KeyguardHostView keyguardView) {
        mKeyguardView = keyguardView;
    }
