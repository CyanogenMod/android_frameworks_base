...
public class PhoneNumberUtils
{
        ...
        if (!TextUtils.isEmpty(emergencyNumbers)) {
            ...
            for (String emergencyNum : emergencyNumbers.split(",")) {
                ...
                if (useExactMatch || "BR".equalsIgnoreCase(defaultCountryIso)) {
                    if (number.equals(emergencyNum) && isEmergencyNumberForCurrentIso(number, defaultCountryIso)) {
                        return true;
                    }
                } else {
                    if (number.startsWith(emergencyNum) && isEmergencyNumberForCurrentIso(number, defaultCountryIso)) {
                        return true;
                    }
                }
            }
   ...
   /**
    * When checking for ECC numbers the country (defaultCountryIso) passed in is not taken into consideration by
    * the function isEmergencyNumberInternal(subId, number, defaultCountryIso, useExactMatchecclist)
    * this causes the function to return TRUE even in the case when the number is not emergency for defaultCountryIso
    * but the device is in a country where the ecclist list has been updated with the current country's ECC #s.
    * For example if device is in INDIA and we are checking for (XYZ, am, 101, XYZ) it will return true since 101
    ...
    */
    private static boolean isEmergencyNumberForCurrentIso(String number, String country) {
        ...
        //@francisg -Lets create a XML file that is in the format of string, numbers .... list. This way we can overload
        //The XML specifically for a particular device. If this XML is not found then this functiona can return
        //true always, otherwise we will check the XML against passed country and the emergency#'s.
