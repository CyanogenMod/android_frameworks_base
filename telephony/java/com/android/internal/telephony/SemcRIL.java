...
public class SemcRIL extends RIL {
        ...
        switch(stateInt) {
            ...
            case 2: {
                if (mIccHandler == null)
                {
                    ...
                    mIccThread.start();
                    mIccHandler = new IccHandler(mIccThread.getLooper());
                }
                mIccHandler.run();
    ...
    class IccHandler extends Handler implements Runnable {
        ...
        public IccHandler (Looper looper) {
            super (looper);
        }
            ...
            switch (paramMessage.what) {
                case EVENT_GET_ICC_STATUS_DONE:
                    ...
                    if (status.getNumApplications() == 0) {
                        ...
                        if (mPhoneType == RILConstants.CDMA_PHONE) {
                            setRadioState(CommandsInterface.RadioState.RUIM_LOCKED_OR_ABSENT);
                        } else {
                        ...
                        switch (app_state) {
                            ...
                            case APPSTATE_READY:
                                switch (app_type) {
                                    ...
                                    case APPTYPE_RUIM:
                                    case APPTYPE_CSIM:
                                        setRadioState(CommandsInterface.RadioState.RUIM_READY);
                                        break;
                                    default:
                                        Log.e(LOG_TAG, "Currently we don't handle SIMs of type: " + app_type);
                                        return;
                ...
                case EVENT_ICC_STATUS_CHANGED:
                    getIccCardStatus(obtainMessage(RIL_REQUEST_GET_SIM_STATUS));
                    break;
                default:
        ...
        public void run () {
            unregisterForIccStatusChanged(this);
            registerForIccStatusChanged(this, EVENT_ICC_STATUS_CHANGED, null);
            getIccCardStatus(obtainMessage(EVENT_GET_ICC_STATUS_DONE));
        }
    }
