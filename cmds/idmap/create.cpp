...
namespace {
    const char *RESOURCES_FILENAME = "resources.arsc";
...
fail:
    ...
    bool is_idmap_stale_fd(const char *target_apk_path, const char *overlay_apk_path, int idmap_fd)
    {
        ...
        if (get_zip_entry_crc(target_apk_path, RESOURCES_FILENAME, &actual_target_crc) == -1) {
            return true;
        }
            ...
            uint32_t **data, size_t *size)
    {
        ...
        if (get_zip_entry_crc(target_apk_path, RESOURCES_FILENAME, &target_crc) == -1) {
            return -1;
        }
