...
int register_android_server_BatteryService(JNIEnv* env)
{
    ...
    while ((entry = readdir(dir))) {
		...
		*/
		...
		//variables for the device test
		bool hasChargeCounter = false;
		char valueChargeCounter[2];
		//test to see if the new value is in build.prop and set bool if it is

		if (property_get("ro.product.chargecounter", valueChargeCounter, NULL)
		    && (!strcmp(valueChargeCounter, "1"))) {
		    hasChargeCounter = true;
		}
