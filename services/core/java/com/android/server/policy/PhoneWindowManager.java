...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        loadDeviceKeyHandlers(deviceKeyHandlerList);
    }

    private void loadDeviceKeyHandlers(final List<Pair<String, String>> list) {
        for (Pair<String, String> pair : list) {
            try {
                PathClassLoader loader = new PathClassLoader(
                        pair.first, getClass().getClassLoader());
                Class<?> klass = loader.loadClass(pair.second);
                Constructor<?> constructor = klass.getConstructor(Context.class);
                mDeviceKeyHandlers[list.indexOf(pair)] =
                        (DeviceKeyHandler) constructor.newInstance(mContext);
            } catch (Exception e) {
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        for (DeviceKeyHandler deviceKeyHandler : mDeviceKeyHandlers) {
            try {
                // The device only should consume known keys.
                if (deviceKeyHandler.handleKeyEvent(event)) {
                    return -1;
                }
            } catch (Exception e) {
