...
public class StatusBarManagerService extends IStatusBarService.Stub {
     ...
     * {@link android.service.statusbar.CustomTileListenerService}.
     * @hide
     */
    @Override
    public void registerListener(final ICustomTileListener listener,
                                 final ComponentName component, final int userid) {
        enforceSystemOrSystemUI("ICustomTileListener.registerListener");
    ...
    int indexOfQsTileLocked(String pkg, String tag, int id, int userId) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            System.out.println("QuickSettingsRecord key " + r.sbTile.getKey());
            System.out.println("QuickSettingsRecord id " + r.sbTile.getId());
            System.out.println("QuickSettingsRecord user id " + r.getUserId());
            if (!customTileMatchesUserId(r, userId) || r.sbTile.getId() != id) {
                System.out.println("QuickSettingsRecord user or id doesn't match");
                continue;
            }
            if (tag == null) {
                if (r.sbTile.getTag() != null) {
                    System.out.println("QuickSettingsRecord tag wasn't null");
                    continue;
                }
            } else {
                if (!tag.equals(r.sbTile.getTag())) {
                    System.out.println("QuickSettingsRecord tags not the same");
                    continue;
                }
            }
            if (r.sbTile.getPackage().equals(pkg)) {
                System.out.println("QuickSettingsRecord package not the same");
                return i;
            }
        }
        return -1;
    ...
    void removeCustomTileWithTagInternal(final int callingUid, final int callingPid,
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                synchronized (mQSTileList) {
                    System.out.println("Remvoing tile " + pkg + " " + " id " + id);
    ...
    public class CustomTileListeners extends ManagedServices {
        ...
        public void notifyRemovedLocked(StatusBarPanelCustomTile sbc) {
            ...
            for (final ManagedServiceInfo info : mServices) {
                System.out.println("Removing tile " + sbc.toString());
                if (!isVisibleToListener(sbcClone, info)) {
                    System.out.println("Not visible to listener");
                    continue;
                }
                mHandler.post(new Runnable() {
                    @Override
        ...
        private void notifyRemoved(ManagedServiceInfo info, StatusBarPanelCustomTile sbc) {
            if (!info.enabledAndUserMatches(sbc.getUserId())) {
                System.out.println("Info not enabled or user matches ");
                return;
            }
            final ICustomTileListener listener = (ICustomTileListener) info.service;
            StatusBarCustomTileHolder sbcHolder = new StatusBarCustomTileHolder(sbc);
