...
public class StatusBarManagerService extends IStatusBarService.Stub {
    ...
    public void createCustomTileWithTag(String pkg, String opPkg, String tag, int id,
            CustomTile customTile, int userId) throws RemoteException {
        ...
        createCustomTileWithTagInternal(pkg, opPkg, Binder.getCallingUid(),
                Binder.getCallingPid(), tag, id, customTile, userId);
    }
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (customTile.getIcon() != 0) {
                    ...
                    mCustomTileListeners.notifyPostedLocked(n, oldSbn);
                } else {
                    Slog.e(TAG, "Not posting quick settings tile with icon==0: " + customTile);
                    if (old != null && !old.isCanceled) {
                        mCustomTileListeners.notifyRemovedLocked(n);
                    }
                }
            }
        });
    ...
    private boolean isVisibleToListener(StatusBarPanelCustomTile sbc,
        ...
        if (!listener.enabledAndUserMatches(sbc.getUserId())) {
            return false;
        }
    ...
    public class CustomTileListeners extends ManagedServices {
        ...
        public CustomTileListeners() {
            super(getContext(), mHandler, mQSTileList, mUserProfiles);
        }
        ...
        public void notifyPostedLocked(StatusBarPanelCustomTile sbc, StatusBarPanelCustomTile oldSbc) {
            ...
            final StatusBarPanelCustomTile sbnClone = null;
            ...
            for (final ManagedServiceInfo info : mServices) {
                ...
                if (oldSbnVisible && !sbnVisible) {
                    final StatusBarPanelCustomTile oldSbnLightClone = oldSbc.clone();
                    mHandler.post(new Runnable() {
                        @Override
                        public void run() {
         ...
         */
        public void notifyRemovedLocked(StatusBarPanelCustomTile sbn) {
            // make a copy in case changes are made to the underlying CustomTile object
            final StatusBarPanelCustomTile sbnLight = sbn.clone();
            for (final ManagedServiceInfo info : mServices) {
        ...
        private void notifyPosted(final ManagedServiceInfo info,
            ...
            final ICustomTileListener listener = (ICustomTileListener)info.service;
            StatusBarCustomTileHolder sbnHolder = new StatusBarCustomTileHolder(sbc);
            try {
                listener.onCustomTilePosted(sbnHolder);
            } catch (RemoteException ex) {
        ...
        private void notifyRemoved(ManagedServiceInfo info, StatusBarPanelCustomTile sbc) {
            if (!info.enabledAndUserMatches(sbc.getUid())) {
                return;
            }
            final ICustomTileListener listener = (ICustomTileListener) info.service;
            StatusBarCustomTileHolder sbnHolder = new StatusBarCustomTileHolder(sbc);
            try {
                listener.onCustomTileRemoved(sbnHolder);
            } catch (RemoteException ex) {
