...
public class LiveDisplayController {
    ...
    private float[] mColorAdjustment = new float[3];

    private float[] mRGB = new float[] { 0.0f, 0.0f, 0.0f };
    ...
    LiveDisplayController(Context context, Handler handler) {
        mContext = context;
        mHandler = handler;
    ...
    private void updateSettings() {
        ...
        try {
            ...
            mColorAdjustment = new float[] { 1.0f, 1.0f, 1.0f };
        }
    ...
    public synchronized void updateLiveDisplay(float lux) {
        mCurrentLux = lux;
        updateLiveDisplayInternal();
    }

    private synchronized void updateLiveDisplayInternal() {
        ...
        Slog.d(TAG, "lux=" + mCurrentLux + " night=" + (mTwilight != null && mTwilight.isNight()) +
                " temperature=" + mColorTemperature + " outdoor=" + mOutdoorMode +
