...
public class LiveDisplayController {
    ...
    private static final String[] COLOR_ADJUSTMENT_DEFAULT = new String[] { "1.0", "1.0", "1.0" };
    ...
    private void updateSettings() {
        ...
        String[] colorAdjustment = colorAdjustmentTemp == null ?
                COLOR_ADJUSTMENT_DEFAULT : colorAdjustmentTemp.split(" ");
        if (colorAdjustment.length != 3) {
            colorAdjustment = COLOR_ADJUSTMENT_DEFAULT;
        }
        try {
    ...
    private synchronized void updateColorTemperature(TwilightState twilight) {
        ...
        mAnimator.setDuration((int)Math.abs(mColorTemperature - temperature));
            ...
            switch (msg.what) {
                case MSG_UPDATE_LIVE_DISPLAY:
                    ...
                    if (transition) {
                        ...
                        sendEmptyMessageDelayed(MSG_UPDATE_LIVE_DISPLAY, DateUtils.MINUTE_IN_MILLIS);
                    }

                    Slog.d(TAG, "lux=" + mCurrentLux + " night=" + (twilight != null && twilight.isNight()) +
                            " transition=" + transition + " temperature=" + mColorTemperature +
                            " outdoor=" + mOutdoorMode + " enhance=" + mColorEnhancement + " cabc=" + mLowPower);
                    break;
            }
        }
    }
