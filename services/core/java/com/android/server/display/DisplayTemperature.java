    ...
    private final class SettingsObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (DISPLAY_TEMPERATURE_MODE_URI.equals(uri) ||
                ...
                if (mMode == MODE_AUTO) {
                    if (mTwilight != null) {
                        updateTwilight(mTwilight);
                    }
                } else if (mMode == MODE_NIGHT) {
                    updateDisplayTemperature(mNightTemperature);
                } else {
                    updateDisplayTemperature(mDayTemperature);
                }
            }  
        }
    }
    
    public synchronized void updateTwilight(TwilightState state) {
        if (state == null) {
            return;
        }
        mTwilight = state;
        if (mMode == MODE_AUTO) {
            updateDisplayTemperature(getTwilightK(state));    
        }
    }
    
    private synchronized void updateDisplayTemperature(float temperature) {
