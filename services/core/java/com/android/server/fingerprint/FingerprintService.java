...
public class FingerprintService extends SystemService {
    ...
    public void onStart() {
        ...
        if (mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_FINGERPRINT)) {
            if (mHal == 0) {
                throw new RuntimeException("FEATURE_FINGERPRINT present, but no Fingerprint HAL loaded!");
            }
        } else if (mHal != 0) {
            throw new RuntimeException("Fingerprint HAL present, but FEATURE_FINGERPRINT is not set!");
        }
    }
