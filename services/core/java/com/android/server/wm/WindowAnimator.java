...
public class WindowAnimator {
    ...
    WindowAnimator(final WindowManagerService service) {
        ...
        final boolean blurSupported = mContext.getResources().getBoolean(
    ...
    private boolean shouldForceHide(WindowState win) {
        ...
        if (winKeyguardPanel == null) {
            keyguardOn &= mKeyguardBlurMode == 0;
        }
    ...
    private void updateWindowsLocked(final int displayId) {
        ...
        if (mKeyguardGoingAway && mKeyguardBlurMode == 0) {
                ...
                if ((win.mAttrs.privateFlags & PRIVATE_FLAG_KEYGUARD) != 0) {
                    if (!winAnimator.mAnimating) {
                        ...
                        winAnimator.mAnimation.setDuration(
                                mKeyguardBlurMode > 0 ? 0 : KEYGUARD_ANIM_TIMEOUT_MS);
                ...
                if (mPolicy.isForceHiding(win.mAttrs)) {
                    ...
                    if (win.isReadyForDisplay()) {
                        if (nowAnimating && win.mWinAnimator.mKeyguardGoingAwayAnimation) {
                            ...
                            mForceHiding = win.isDrawnLw() && mKeyguardBlurMode == 0 ?
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void observe(Context context) {
            context.getContentResolver().registerContentObserver(
                    CMSettings.Secure.getUriFor(CMSettings.Secure.LOCK_SCREEN_BLUR),
                    false, this);
            onChange(true);
        }
        ...
        public void onChange(boolean selfChange) {
            mKeyguardBlurMode = CMSettings.Secure.getInt(mContext.getContentResolver(),
                    CMSettings.Secure.LOCK_SCREEN_BLUR, 2);
        }
    }
}
