...
;
    ...
    void createNewUserLILPw(PackageManagerService service, Installer installer,
        ...
        for (PackageSetting ps : mPackages.values()) {
            ...
            // Only system apps or prebundled apps are initially installed.
            boolean setInstalled = ((ps.pkgFlags&ApplicationInfo.FLAG_SYSTEM) != 0);

            Log.d(PackageManagerService.TAG, "set Installed " + setInstalled + " name " + ps.name + " package " + ps.getInstallerPackageName());

            //setInstalled = !wasPrebundledPackageInstalledLPr(UserHandle.USER_OWNER,
            //        ps.getInstallerPackageName()
