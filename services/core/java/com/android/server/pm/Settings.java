...
;
    ...
    void updateUnusedPermissions() {
        ...
        for (BasePermission basePerm : mPermissions.values()) {
            ...
            // Only remove normal permissions
            if (basePerm.protectionLevel != PermissionInfo.PROTECTION_NORMAL) {
                continue;
            }
        ...
        // And now remove all unused permission
        for (BasePermission basePerm : permissionsToRemove) {
            String msg = "Removed unused permission: " + basePerm.name + " original " +
                    "asigned to " + basePerm.sourcePackage + "\n";
            mReadMessages.append(msg);
            PackageManagerService.reportSettingsProblem(Log.WARN, msg);
