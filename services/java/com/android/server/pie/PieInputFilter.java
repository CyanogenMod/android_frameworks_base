...
public class PieInputFilter implements IInputFilter {
        ...
        try {
            ...
            if (deviceId != mDeviceId) {
                ...
                if ((policyFlags & WindowManagerPolicy.FLAG_PASS_TO_USER) == 0) {
                    synchronized (mLock) {
                        clearAndResetStateLocked(false);
                    }
                }
    ...
    private void clearAndResetStateLocked(boolean force) {
        // ignore soft reset in POSTSYNTHESIZE and SYNTHESIZE, because we need to tamper with
        // the event stream and going to LISTEN after an ACTION_UP anyway
        if (!force && (mState == State.POSTSYNTHESIZE || mState == State.SYNTHESIZE)) {
            return;
        }
        switch (mState) {
            case LISTEN:
