...
public class PieGestureTracker {
    ...
    public PieGestureTracker(int thickness, int distance) {
        if (DEBUG) {
    ...
    public boolean move(MotionEvent motionEvent) {
        ...
        switch (mPosition) {
            case LEFT:
                ...
                if (deltaY < mTriggerDistance && deltaY > -mTriggerDistance && x >= mLastX) {
                ...
                break;
            case BOTTOM:
                if (deltaX < mTriggerDistance && deltaX > -mTriggerDistance && y <= mLastY) {
        ...
        if (loaded && mActivationListener != null) {
            if (DEBUG) {
                Slog.d(TAG, "activate at " + x + "," + y + " " + mPosition);
            }
