...
public class BroadcastQueue {
    ...
    private final void deliverToRegisteredReceiverLocked(BroadcastRecord r,
        ...
        try {
            if (!skip && mService.isFilteredByIncognitoMode(r.intent.getAction()) &&
                    ActivityManagerNative.getDefault().isProcessIncognito(filter.receiverList.pid)) {
                Slog.w(TAG, "Skipping broadcast of "
                        + r.intent.toString()
                        + " to " + filter.receiverList.app
                        + " (pid=" + filter.receiverList.pid
                        + ") due to incognito mode");
                skip = true;
            }
