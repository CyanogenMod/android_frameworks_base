...
public final class Installer {
    ...
    private String transaction(String cmd) {
        ...
        try {
            ...
            while(!checkForTransaction(transactionId)) {
    ...
    private void checkPoller() {
        synchronized (mPendingRequests) {
            Log.v(TAG, "checkPoller handler = "+mHandler+" empty="+mPendingRequests.isEmpty());
            if (mHandler != null) {
    ...
    private boolean checkForTransaction(int id) {
        synchronized (mResponses) {
            return mResponses.get(id) != null;
        }
    }
...
//            Log.v(TAG, "poll runnable");
    ...
    public int dexoptAsync(String apkPath, int uid, boolean isPublic) {
        StringBuilder builder = new StringBuilder("dexopt");
        builder.append(' ');
        builder.append(apkPath);
        builder.append(' ');
