...
public final class BatteryService extends IBatteryService.Stub {
    ...
    private boolean isPoweredLocked(int plugTypeSet) {
        ...
        if ((plugTypeSet & BatteryManager.BATTERY_PLUGGED_DOCK_AC) != 0 && mBatteryProps.chargerDockAcOnline) {
            return true;
        }
    ...
    @Override
    public boolean isDockSupported() throws RemoteException {
        synchronized (mLock) {
            return mDockBatterySupported;
        }
    }
    ...
    public boolean isDockBatteryLow() {
        synchronized (mLock) {
            return mBatteryProps.dockBatteryPresent && mBatteryProps.dockBatteryLevel <= mLowBatteryWarningLevel;
        }
    }
    ...
    private void processValuesLocked() {
        ...
        if (mBatteryProps.chargerAcOnline) {
            mPlugType = mPlugType | BatteryManager.BATTERY_PLUGGED_AC;
        }
        if (mBatteryProps.chargerDockAcOnline) {
            mPlugType = mPlugType | BatteryManager.BATTERY_PLUGGED_DOCK_AC;
        }
        if (mBatteryProps.chargerUsbOnline) {
            mPlugType = mPlugType | BatteryManager.BATTERY_PLUGGED_USB;
        }
        if (mBatteryProps.chargerWirelessOnline) {
            mPlugType = mPlugType | BatteryManager.BATTERY_PLUGGED_WIRELESS;
        }
        ...
        if (DEBUG) {
            ...
            if (mDockBatterySupported) {
                dbg += ", dockBatteryHealth=" + mBatteryProps.dockBatteryHealth
        ...
        if (mDockBatterySupported) {
            try {
                mBatteryStats.setDockBatteryState(mBatteryProps.dockBatteryStatus,
                        mBatteryProps.dockBatteryHealth, mPlugType, mBatteryProps.dockBatteryLevel,
                        mBatteryProps.dockBatteryTemperature, mBatteryProps.dockBatteryVoltage);
            } catch (RemoteException e) {
                // Should never happen.
            }
        }
        ...
        if (mBatteryProps.batteryStatus != mLastBatteryStatus ||
            ...
            if (mDockBatterySupported && (mBatteryProps.dockBatteryStatus != mLastDockBatteryStatus ||
                    mBatteryProps.dockBatteryHealth != mLastDockBatteryHealth ||
                    mBatteryProps.dockBatteryPresent != mLastDockBatteryPresent ||
                    mPlugType != mLastPlugType)) {
                EventLog.writeEvent(EventLogTags.DOCK_BATTERY_STATUS,
                        ...
                        mPlugType, mBatteryProps.dockBatteryTechnology);
            }
            ...
            if (mDockBatterySupported && (mBatteryProps.dockBatteryLevel != mLastDockBatteryLevel)) {
                // Don't do this just from voltage or temperature changes, that is
                // too noisy.
    ...
    private void sendIntentLocked() {
        ...
        if (mDockBatterySupported) {
            ...
            intent.putExtra(BatteryManager.EXTRA_DOCK_PLUGGED,
                    (mPlugType & BatteryManager.BATTERY_PLUGGED_DOCK_AC) == BatteryManager.BATTERY_PLUGGED_DOCK_AC
                            ? BatteryManager.BATTERY_PLUGGED_DOCK_AC
                            : BATTERY_PLUGGED_NONE);
            intent.putExtra(BatteryManager.EXTRA_DOCK_VOLTAGE, mBatteryProps.dockBatteryVoltage);
            intent.putExtra(BatteryManager.EXTRA_DOCK_TEMPERATURE, mBatteryProps.dockBatteryTemperature);
            intent.putExtra(BatteryManager.EXTRA_DOCK_TECHNOLOGY, mBatteryProps.dockBatteryTechnology);

            // EEPAD legacy data
            intent.putExtra("usb_wakeup", (mPlugType & BatteryManager.BATTERY_PLUGGED_USB)
                    == BatteryManager.BATTERY_PLUGGED_USB);
            intent.putExtra("ac_online", (mPlugType & BatteryManager.BATTERY_PLUGGED_AC)
                    == BatteryManager.BATTERY_PLUGGED_AC);
            intent.putExtra("dock_ac_online", (mPlugType & BatteryManager.BATTERY_PLUGGED_DOCK_AC)
                    == BatteryManager.BATTERY_PLUGGED_DOCK_AC);
        }
            ...
            if (args == null || args.length == 0 || "-a".equals(args[0])) {
                ...
                if (mDockBatterySupported) {
                    ...
                    if (mBatteryProps.dockBatteryChargeCounter != Integer.MIN_VALUE) {
                        pw.println("  dock charge counter: " + mBatteryProps.dockBatteryChargeCounter);
                    }
                ...
                if (mDockBatterySupported) {
                    pw.println("  set ac|dockac|usb|wireless|status|level|dockstatus|docklevel|invalid <value>");
                } else {
