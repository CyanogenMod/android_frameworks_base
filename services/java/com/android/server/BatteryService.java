...
class BatteryService extends Binder {
    ...
    class Led {
        ...
        void updateLightsLocked() {
            ...
            if (!mLightEnabled || (inQuietHours() && mQuietHoursDim)) {
                // If the light is enabled, the battery is low, the device is not charging and
                // the user has enabled the low battery pulse - ignore Quiet Hours
                if (mLightEnabled && mLedPulseEnabled && level < mLowBatteryWarningLevel &&
                        status != BatteryManager.BATTERY_STATUS_CHARGING) {
                    mBatteryLight.setFlashing(mBatteryLowARGB, LightsService.LIGHT_FLASH_TIMED,
                            mBatteryLedOn, mBatteryLedOff);
                } else {
                    // No lights if explicitly disabled or in Quiet Hours and battery not low
                    mBatteryLight.turnOff();
                }
            } else {
                // Normal battery low / battery charging handling
                if (level < mLowBatteryWarningLevel) {
                    if (status == BatteryManager.BATTERY_STATUS_CHARGING) {
                        // Color when battery is charging and low
                        mBatteryLight.setColor(mBatteryLowARGB);
                    } else if (mLedPulseEnabled) {
                        // Flash low color when battery is low and not charging
                        mBatteryLight.setFlashing(mBatteryLowARGB, LightsService.LIGHT_FLASH_TIMED,
                                mBatteryLedOn, mBatteryLedOff);
                    } else {
                    ...
                    if (status == BatteryManager.BATTERY_STATUS_FULL || level >= 90) {
                        // Color when full or charging and nearly full
                        mBatteryLight.setColor(mBatteryFullARGB);
                    } else {
                        // Color when charging and halfway full
