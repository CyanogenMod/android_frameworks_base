...
//        lp.token = mStatusBarView.getWindowToken();
   ...
   private View.OnClickListener mPowerListener = new View.OnClickListener() {
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		BluetoothButton btn = (BluetoothButton)mUsedPowerButtons.get(PowerButton.TOGGLE_BLUETOOTH);
                if (btn != null) btn.onReceive(context, intent);
            } else if (NetworkModeButton.NETWORK_MODE_CHANGED.equals(action)) {
		NetworkModeButton btn = (NetworkModeButton)mUsedPowerButtons.get(PowerButton.TOGGLE_NETWORKMODE);
                if (btn != null) btn.onReceive(context, intent);
            }
            updateWidget();
        }
