...
public class AutoBrightnessDeviceHandler {
    ...
    private int mPrevDevSettingsAutoBrightness = 0;
 ...
 // Our context
    ...
    private void enableAutoBrightness() {
        writeValue(FILE, AUTO_INDOOR1); // will be later properly updated by onUpdateAutoBrightness()
        Slog.d(TAG, "AutoBrightness re-enabled");
    }
    ...
    private void disableAutoBrightness() {
        ...
        if (mSysfsFileExists) {
            sysfsValue = readInt(FILE);
            Slog.d(TAG, "Current sysfsValue" + String.valueOf(sysfsValue));
            if ( sysfsValue >= AUTO_INDOOR1) {
                /* Autobrightness was allowed in Device settings so we'll remember this setting to re-enable it if
                 * autobrightness is enabled again */
                mPrevDevSettingsAutoBrightness = sysfsValue;
            }
    ...
    private void toggleAutoBrightness() {
        ...
        if ((Settings.System.getIntForUser(cr, Settings.System.SCREEN_BRIGHTNESS_MODE, Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL, UserHandle.USER_CURRENT) == Settings.System.SCREEN_BRIGHTNESS_MODE_AUTOMATIC) &&
           (mPrevDevSettingsAutoBrightness >= AUTO_INDOOR1)){
