...
import com.android.internal.util.cm.ImageUtility;
...
public class ThemeService extends IThemeService.Stub {
    ...
    private boolean setCustomLockScreenWallpaper(String pkgName) {
        ...
        try {
            if (HOLO_DEFAULT.equals(pkgName)) {
                ...
                InputStream in = ImageUtility.getCroppedKeyguardStream(pkgName, mContext);
                if (in != null) {
    ...
    private boolean updateWallpaper(String pkgName) {
        ...
        if (HOLO_DEFAULT.equals(pkgName)) {
            ...
            InputStream in = null;
            try {
                in = ImageUtility.getCroppedWallpaperStream(pkgName, mContext);
                if (in != null)
                    wm.setStream(in);
            } catch (Exception e) {
                return false;
