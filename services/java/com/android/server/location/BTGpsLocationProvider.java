...
public class BTGpsLocationProvider  implements LocationProviderInterface {
	...
	public BTGpsLocationProvider(Context context, ILocationManager locationManager) {
        ...
        // Create a wake lock. prob not needed.
                ...
                if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                    ...
                    switch (state) {
                    	...
                    	case BluetoothAdapter.STATE_TURNING_OFF:
                    		if (btsvc.getState() != BTGPSService.STATE_NONE) {
    ...
    private synchronized void handleEnable() {
    	...
    	if (!mAdapter.isEnabled()) {
    		...
    		int tmp = mAdapter.getState();
    		if (tmp == BluetoothAdapter.STATE_OFF) {
    		...
    		while (true) {
    			try {
    				tmp = mAdapter.getState();
    				if (tmp == BluetoothAdapter.STATE_ON) 
    					break;
    				else if (tmp == BluetoothAdapter.STATE_TURNING_ON) {
        ...
        if (btDevice != null && btDevice != "0") {
        	if (mAdapter != null && mAdapter.isEnabled()) {
	           	ArrayList<BluetoothDevice> tmp = (new ArrayList<BluetoothDevice>(mAdapter.getBondedDevices()));
    ...
    private void reportLocation(Location loc, boolean isValid) {
        ...
        synchronized (mLocation) {
        	...
        	if (D) Log.v(TAG, "reportLocation lat: " + mLocation.getLatitude() + " long: " + mLocation.getLongitude() +
                    " alt: " + mLocation.getAltitude() + " accuracy: " + mLocation.getAccuracy() + " timestamp: " + mLocation.getTime());
        	try {
	...
	*/ 
            ...
            for (int i = 0; i < size; i++) {
                ...
                try {
                    listener.mListener.onSvStatusChanged(svCount, mSvs, mSnrs, 
                            mSvElevations, mSvAzimuths, mSvMasks[NMEAParser.EPHEMERIS_MASK], 
                            mSvMasks[NMEAParser.ALMANAC_MASK], mSvMasks[NMEAParser.USED_FOR_FIX_MASK]); 
                } catch (RemoteException e) {
    ...
    private void notifyEnableDisableGPS(boolean status) {
        ...
        synchronized(mListeners) {
        	...
        	//LocationManager.GPS_FIX_CHANGE_ACTION
        	mContext.sendBroadcast(intent);
