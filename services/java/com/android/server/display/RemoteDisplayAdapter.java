...
public class RemoteDisplayAdapter extends DisplayAdapter {
    ...
    class Stub extends IRemoteDisplayAdapter.Stub {
        public void registerDisplayDevice(IDisplayDevice displayDevice, String name,
        ...
        public void disconnectRemoteDisplay() {
            synchronized (getSyncRoot()) {
                if (mActiveDisplay != null && mActiveDisplay.remoteDisplayDevice != null) {
                    ...
                    sendDisplayDeviceEventLocked(mActiveDisplay.remoteDisplayDevice, DisplayAdapter.DISPLAY_DEVICE_EVENT_REMOVED);
        ...
        public WifiDisplayStatus getRemoteDisplayStatus() {
            synchronized (getSyncRoot()) {
                ...
                for (RemoteDisplay display: mDevices.values()) {
                    WifiDisplay add = new WifiDisplay(display.info.address, display.info.name, display.info.name, display.hidden);
                    availableDisplays.add(add);
                }

                WifiDisplay active = null;
                if (mActiveDisplay != null)
                    active = new WifiDisplay(mActiveDisplay.info.address, mActiveDisplay.info.name, mActiveDisplay.info.name);

                WifiDisplayStatus status = new WifiDisplayStatus(WifiDisplayStatus.FEATURE_STATE_ON,
                    mScanning ? WifiDisplayStatus.SCAN_STATE_SCANNING : WifiDisplayStatus.SCAN_STATE_NOT_SCANNING,
