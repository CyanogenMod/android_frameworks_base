    ...
    private void setWeatherData(WeatherInfo w) {
        ...
        if (mWeatherPanel != null) {
            ...
            if (mWeatherUpdateTime != null) {
                ...
                int lsDay = (int) Math.floor(w.last_sync / 86400000);
                int nDay = (int) Math.floor(now / 86400000);
                if (nDay - lsDay >= 2) {
                    lastTimeText = String.format(getContext().getString(R.string.weather_last_sync_days), nDay - lsDay);
                } else if (nDay - lsDay >= 1) {
                    lastTimeText = String.format(getContext().getString(R.string.weather_last_sync_yesterday),
                            DateFormat.getTimeFormat(getContext()).format(lastTime));
                } else {
                    lastTimeText = String.format(getContext().getString(R.string.weather_last_sync_today),
                            DateFormat.getTimeFormat(getContext()).format(lastTime));
                }
