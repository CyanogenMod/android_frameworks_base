...
public class KeyguardUpdateMonitor {
    ...
    //
    // Returns the time value - how long should be screen keep not securely locked,
    // used by pattern lock timeout
    //
    public long getUnlockedTimeout() {
        long ret = 0;

        long nSecureTimeout = (long)Settings.Secure.getInt(
        ...
        if ( (nSecureTimeout != 0)
            ...
            if (now - mTimeLastScreenLock < nSecureTimeout) {
                ret = mTimeLastScreenLock + nSecureTimeout - now;
            }
        }
    ...
    public long getUnlockedUntil() {
        long ret = 0;
        long timeout = getUnlockedTimeout();
        if (timeout > 0)
            ret = SystemClock.elapsedRealtime() + timeout;
        return ret;
    }
