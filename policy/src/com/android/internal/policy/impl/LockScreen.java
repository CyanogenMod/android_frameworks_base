...
class LockScreen extends LinearLayout implements KeyguardScreen,
		...
		RotarySelector.OnDialTriggerListener, OnGesturePerformedListener {
	...
	private AudioManager am = (AudioManager) getContext().getSystemService(
	...
	private boolean mTrackballUnlockScreen = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.TRACKBALL_UNLOCK_SCREEN, 0) == 1);

	private boolean mMenuUnlockScreen = (Settings.System.getInt(
			mContext.getContentResolver(), Settings.System.MENU_UNLOCK_SCREEN,
			0) == 1);

	private boolean mLockAlwaysBattery = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_ALWAYS_BATTERY, 0) == 1);

	private boolean mLockMusicControls = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_MUSIC_CONTROLS, 1) == 1);

	private boolean mNowPlayingToggle = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_NOW_PLAYING, 1) == 1);

	private boolean mAlbumArtToggle = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_ALBUM_ART, 1) == 1);

	private int mLockMusicHeadset = (Settings.System.getInt(
			mContext.getContentResolver(),
	...
	private boolean mLockAlwaysMusic = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_ALWAYS_MUSIC_CONTROLS, 0) == 1);

	private boolean mCustomAppToggle = (Settings.System.getInt(
			mContext.getContentResolver(),
	...
	private int mLockscreenStyle = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_STYLE_PREF, 3));

	private int mCustomIconStyle = Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_CUSTOM_ICON_STYLE, 1);

	private boolean mRotaryUnlockDown = (Settings.System.getInt(
			mContext.getContentResolver(),
			Settings.System.LOCKSCREEN_ROTARY_UNLOCK_DOWN, 0) == 1);

	private boolean mRotaryHideArrows = (Settings.System.getInt(
			mContext.getContentResolver(),
	...
	LockScreen(Context context, Configuration configuration,
		...
		mNowPlaying.setSelected(true); // set focus to TextView to allow
										// scrolling
		...
		mAlbumArt.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				musicIntent.setClassName("com.android.music",
		...
		if (!mRotaryUnlockDown) {
			...
			mRotarySelector
					.setMidHandleResource((mCustomIconStyle == 1) ? R.drawable.ic_jog_dial_custom
							: R.drawable.ic_jog_dial_messaging);
		} else {
			mRotarySelector
					.setLeftHandleResource((mCustomIconStyle == 1) ? R.drawable.ic_jog_dial_custom
							: R.drawable.ic_jog_dial_messaging);
		...
		mTabSelector.setLeftTabResources(R.drawable.ic_jog_dial_unlock,
		...
		if (mSelector2 != null) {
			...
			mSelector2.setRightTabResources(
					(mCustomIconStyle == 1) ? R.drawable.ic_jog_dial_custom
							: R.drawable.ic_jog_dial_messaging,
					R.drawable.jog_tab_target_green,
			...
			mSelector2.setOnTriggerListener(new OnTriggerListener() {
				public void onTrigger(View v, int whichHandle) {
					if (whichHandle == SlidingTab.OnTriggerListener.LEFT_HANDLE) {
						...
						if (mCustomAppActivity != null) {
							try {
								Intent i = Intent.parseUri(mCustomAppActivity,
										0);
		...
		boolean prefHideUnlockTab = (Settings.Secure.getInt(
		...
		if (gestures != null) {
			if (mGestureActive) {
				...
				mGestureSensitivity = Settings.System.getInt(
						context.getContentResolver(),
		...
		// tab don't properly update system settings... ALWAYS provide a way to
		// unlock the phone
		if (prefHideUnlockTab
				&& (GestureCanUnlock || mTrackballUnlockScreen || mMenuUnlockScreen)) {
			mHideUnlockTab = true;
		} else {
	...
	private void refreshBatteryStringAndIcon() {
		...
		if (mPluggedIn) {
			...
			if (mBatteryLevel <= 20) {
				...
				mCharging = getContext().getString(
						R.string.lockscreen_low_battery, mBatteryLevel);
			} else {
				mChargingIcon = getContext().getResources().getDrawable(
	...
	private void refreshMusicStatus() {
		if ((mWasMusicActive
				|| mIsMusicActive
				|| mLockAlwaysMusic
				|| (mAudioManager.isWiredHeadsetOn() && useLockMusicHeadsetWired) || (mAudioManager
				.isBluetoothA2dpOn() && useLockMusicHeadsetBT))
				&& (mLockMusicControls)) {
			if (am.isMusicActive()) {
				mPauseIcon.setVisibility(View.VISIBLE);
				mPlayIcon.setVisibility(View.GONE);
	...
	private void refreshPlayingTitle() {
		...
		if (am.isMusicActive() && !nowPlaying.equals("") && mLockMusicControls
			...
			// Set album art
			Uri uri = getArtworkUri(getContext(),
					KeyguardViewMediator.SongId(),
	...
	private void updateLayout(Status status) {
		...
		mCustomAppToggle = (Settings.System.getInt(
				mContext.getContentResolver(),
				Settings.System.LOCKSCREEN_CUSTOM_APP_TOGGLE, 0) == 1);
		mRotarySelector.enableCustomAppDimple(mCustomAppToggle);
	...
	static CharSequence getCarrierString(CharSequence telephonyPlmn,
		...
		if (telephonyPlmn != null
				&& (telephonySpn == null || "".contentEquals(telephonySpn))) {
			return telephonyPlmn;
		} else if (telephonySpn != null
				&& (telephonyPlmn == null || "".contentEquals(telephonyPlmn))) {
			return telephonySpn;
	...
	private void toggleSilentMode() {
		...
		final boolean mVolumeControlSilent = Settings.System.getInt(
				mContext.getContentResolver(),
		...
		mSilentMode = mVolumeControlSilent ? ((mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_VIBRATE) || !mSilentMode)
				: !mSilentMode;
		if (mSilentMode) {
			final boolean vibe = mVolumeControlSilent ? (mAudioManager
	...
	public void onGesturePerformed(GestureOverlayView overlay, Gesture gesture) {
		...
		if (predictions.size() > 0
				&& predictions.get(0).score > mGestureSensitivity) {
			...
			mCallback.pokeWakelock(); // reset timeout - give them another
										// chance to gesture
		}
	}
