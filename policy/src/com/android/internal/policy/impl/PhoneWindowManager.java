...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    Runnable mVolumePlusLongPress = new Runnable() {
        public void run() {
            mVolPlusLong=true;
            interceptVolumeKeysBeforeQueueing(CmSystem.KEYCODE_NONE, true);
        }
    };
    ...
    private void handleVolumeActions(int keyCode, int customAction, boolean useCustomAction)
    {
        ...
        if(!useCustomAction){
            if(keyCode!=CmSystem.KEYCODE_NONE)
                sendHwButtonEvent(keyCode);
            return;
        }
    ...
    private void interceptVolumeKeysBeforeQueueing(int keyCode, boolean down) {
         ...
         */
        boolean callbackMode = (keyCode==CmSystem.KEYCODE_NONE);
        ...
        if(!mScreenOn && mVolBtnMusicControls && callbackMode && (mVolPlusLong || mVolMinusLong))
            handleVolumeActions(CmSystem.KEYCODE_NONE, mVolPlusLong ? ACTION_MEDIA_NEXT : ACTION_MEDIA_PREVIOUS, true);
        ...
        if(wasVolBothDown && up){
            handleVolumeActions(CmSystem.KEYCODE_NONE, mVolBothAction, true);
            wasVolBothDown=false;
        }
        if((mVolMinusLong && mVolPlusDown) || (mVolPlusLong && mVolMinusDown))
            handleVolumeActions(CmSystem.KEYCODE_NONE, mLongVolBothAction, true);
