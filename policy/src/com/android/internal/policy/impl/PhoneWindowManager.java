...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private PieManager.PieActivationListener mShowStatusBarListener =
        ...
        public void onPieActivation(int touchX, int touchY, PiePosition position, int flags) {
            if (!mStatusBarShownDueToGesture) {
                dropNextEvents();
                setStatusBarShownDueToGesture(true);
            ...
            switch (m.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    if (!isStatusbarExpanded() && m.getY() > mStatusBarHeight) {
                        setStatusBarShownDueToGesture(false);
                        mWindowManagerFuncs.reevaluateStatusBarVisibility();
                    } else {
                        delayHiding();
                    }
    ...
    private void enableStatusBarShowPieListener() {
        PieManager pieManager = PieManager.getInstance();
        pieManager.updatePieActivationListener(mShowStatusBarListener,
                3 << PieServiceConstants.SENSITIVITY_SHIFT | PiePosition.TOP.FLAG);
    }

    private void disableStatusBarShowPieListener() {
        PieManager pieManager = PieManager.getInstance();
        pieManager.updatePieActivationListener(mShowStatusBarListener, 0);
    }
    ...
    private void setStatusBarShownDueToGesture(boolean shown) {
        mStatusBarShownDueToGesture = shown;
        if (shown) {
    ...
    public void systemReady() {
        ...
        synchronized (mLock) {
            PieManager pieManager = PieManager.getInstance();
            pieManager.setPieActivationListener(mShowStatusBarListener);
            updateOrientationListenerLp();
            mSystemReady = true;
            updateShowStatusBarReceiverState();
            mHandler.post(new Runnable() {
