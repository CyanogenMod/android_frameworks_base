...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final int VOLUME_ACTION_LONG_HOME = -1;
    static final int VOLUME_ACTION_NONE = 0;
    static final int KEYCODE_NONE = -23;
    ...
    // variables connected to volume key states - should be merged into a class
    // on bugfixing
    boolean mVolMinusDown;
    boolean mVolPlusDown;
    boolean mVolMinusLong;
    boolean mVolPlusLong;
    boolean mWasActionDone;
    ...
    private void sendHwButtonEvent(int keycode) {
        try {
    ...
    private int translateActionToKeycode(int longActionIndex) {
        switch (longActionIndex){
            ...
            case ACTION_MEDIA_NEXT: return KeyEvent.KEYCODE_MEDIA_NEXT;
            case ACTION_MEDIA_PREVIOUS: return KeyEvent.KEYCODE_MEDIA_PREVIOUS;
            case 11: return VOLUME_ACTION_LONG_HOME;
            default: return VOLUME_ACTION_NONE;
        }
    }

    private int translateUnhideIndexToKeycode(int unhideButtonIndex) {
        switch (unhideButtonIndex){
            ...
            default: return KeyEvent.KEYCODE_POWER;
        }
    }
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            mLongVolBothAction = Settings.System.getInt(resolver,
                    Settings.System.LONG_VOL_BOTH_ACTION, 0);
            mUnhideKeyCode = translateUnhideIndexToKeycode(Settings.System.getInt(resolver,
    ...
    public int interceptKeyBeforeQueueing(long whenNanos, int action, int flags,
        ...
        // return 0 used, since there is no ACTION_DO_NOTHING defined
        if(hasHandledUnhideButton(keyCode, down))
            return 0;
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_UP: {
                if(!isInjected){
                    interceptVolumeKeysBeforeQueueing(keyCode, down);
                    result &= ~ACTION_PASS_TO_USER;
                    if(keyguardActive||!mScreenOn)
    ...
    private boolean hasHandledUnhideButton(int keyCode, boolean down){
        ...
        if(keyCode!=mUnhideKeyCode && (mUnhideKeyCode==KeyEvent.KEYCODE_POWER
    ...
    Runnable mVolumePlusLongPress = new Runnable() {
        public void run() {
            mVolPlusLong=true;
            interceptVolumeKeysBeforeQueueing(KEYCODE_NONE, true);
        }
    };

    Runnable mVolumeMinusLongPress = new Runnable() {
        public void run() {
            mVolMinusLong=true;
            interceptVolumeKeysBeforeQueueing(KEYCODE_NONE, true);
        }
    ...
    private void handleVolumeActions(int keyCode, int customAction, boolean useCustomAction)
    {
        if(mWasActionDone)
            return;
        mWasActionDone=true;
        ...
        int translated=translateActionToKeycode(customAction);

        if(translated==VOLUME_ACTION_LONG_HOME)
    ...
    private void interceptVolumeKeysBeforeQueueing(int keyCode, boolean down) {
        /**
         * Part 1 of 4: Setup of later needed variables
         */
        boolean callbackMode = (keyCode==KEYCODE_NONE);
        boolean up = (!callbackMode && !down);
        boolean wasVolBothDown=false;

        // get ringing and phone status
        ITelephony telephonyService = getTelephonyService();
        ...
        /**
         * Part 2 of 4: Determine the actual volume key status
         */
        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN){
            ...
            if(mVolPlusDown)
                wasVolBothDown=true;
            if(down){
                mWasActionDone=false;
                mVolMinusDown=true;
                mHandler.postDelayed(mVolumeMinusLongPress, ViewConfiguration.getLongPressTimeout());
            }else{
                mVolMinusDown=false;
                mHandler.removeCallbacks(mVolumeMinusLongPress);
            }
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP){
            ...
            if(down){
                mWasActionDone=false;
                mVolPlusDown=true;
                mHandler.postDelayed(mVolumePlusLongPress, ViewConfiguration.getLongPressTimeout());
            }else{
                mVolPlusDown=false;
                mHandler.removeCallbacks(mVolumePlusLongPress);
            }
        }

        /**
         * Part 3 of 4: Keyguard & volume music control
         */
        if(!mScreenOn && mVolBtnMusicControls && callbackMode && (mVolPlusLong || mVolMinusLong))
            handleVolumeActions(KEYCODE_NONE, mVolPlusLong ? ACTION_MEDIA_NEXT : ACTION_MEDIA_PREVIOUS, true);

        final boolean keyguardActive = (mScreenOn ?
                mKeyguardMediator.isShowingAndNotHidden() :
                mKeyguardMediator.isShowing());
        if(keyguardActive)
            return;
        ...
        /**
         * Part 4 of 4: Handling all cases that can happen on volume keys
         */
        // both keys pressed at once
        if(wasVolBothDown && up){
            handleVolumeActions(KEYCODE_NONE, mVolBothAction, true);
            wasVolBothDown=false;
        }
        if((mVolMinusLong && mVolPlusDown) || (mVolPlusLong && mVolMinusDown))
            handleVolumeActions(KEYCODE_NONE, mLongVolBothAction, true);

        // short normal keypress - react on up
        if(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN && up)
            handleVolumeActions(keyCode, mLongVolMinusAction, mRevVolBehavior);
        if(keyCode == KeyEvent.KEYCODE_VOLUME_UP && up)
            handleVolumeActions(keyCode, mLongVolPlusAction, mRevVolBehavior);

        // long keypress for normal keys
        if(mVolMinusLong && !mVolPlusDown && callbackMode)
            handleVolumeActions(keyCode, mLongVolMinusAction, !mRevVolBehavior);
        if(mVolPlusLong && !mVolMinusDown && callbackMode)
            handleVolumeActions(keyCode, mLongVolPlusAction, !mRevVolBehavior);

        // press while ringing
        if (isRinging && down && !callbackMode) {
        ...
        if (isOffhook && down && !callbackMode) {
