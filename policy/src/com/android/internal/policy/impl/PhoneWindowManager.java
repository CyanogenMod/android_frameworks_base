...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private final class ShowStatusBarGestureReceiver extends InputEventReceiver {
        ...
        private float MIN_VELOCITY = 450; // 450 dp/sec
        private float MIN_DISTANCE = 120; // 120 dp
        public ShowStatusBarGestureReceiver(InputChannel inputChannel,
            ...
            mHandler = new Handler(looper) {
                ...
                public void handleMessage(Message msg) {
                    if (MSG_HIDE_STATUS_BAR == msg.what) {
                        mUserGestureShow = false;
                        mWindowManagerFuncs.reevaluateStatusBarVisibility();
                    }
                }
            };
            final float multiplier = mContext.getResources().getDisplayMetrics().density;
            MIN_DISTANCE = MIN_DISTANCE * multiplier;
            MIN_VELOCITY = MIN_VELOCITY * multiplier;
        }
        ...
        public void onInputEvent(InputEvent event) {
            ...
            if (event instanceof MotionEvent) {
                ...
                case MotionEvent.ACTION_DOWN:
                    if ((mTopIsFullscreen || mExpandedDesktopStyle == 3)
                            && mHideNavFakeWindow == null
                            && !mUserGestureShow && m.getY() < mStatusBarHeight * 3) {
                        mIsTracking = true;
                        mStartY = m.getY();
                    }
                    break;
                case MotionEvent.ACTION_MOVE:
                    if (mIsTracking) {
                        if (m.getY() - mStartY > MIN_DISTANCE) {
                            mUserGestureShow = true;
                            changed = true;
                            mHandler.removeMessages(MSG_HIDE_STATUS_BAR);
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            mShowStatusBarOnTopOfFullscreen = Settings.System.getIntForUser(resolver,
                    Settings.System.STATUS_BAR_ON_TOP_OF_FULLSCREEN, 1, UserHandle.USER_CURRENT) != 0;

            if (expandedDesktopStyle != mExpandedDesktopStyle) {
                if (expandedDesktopStyle == 3) {
                    mHandler.sendEmptyMessage(MSG_ENABLE_STATUS_BAR_HIDE_RECEIVER);
                } else if (mExpandedDesktopStyle == 3) {
                    mHandler.sendEmptyMessage(MSG_DISABLE_STATUS_BAR_HIDE_RECEIVER);
                }
                if (expandedDesktopStyle == 3 || expandedDesktopStyle == 2) {
                    mShowStatusBarOnTopOfFullscreen = false;
                }
    ...
    private boolean mShowStatusBarOnTopOfFullscreen = true;
    private boolean mUserGestureShow;
    ...
    private boolean expandedDesktopHidesStatusBar() {
        return mExpandedDesktopStyle == 2 || mExpandedDesktopStyle == 3;
    }
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mStatusBar != null) {
            ...
            if (expandedDesktopHidesStatusBar() && !mUserGestureShow) {
                ...
                // case though.
                if (topIsFullscreen && !mUserGestureShow) {
        ...
        if (topIsFullscreen != mTopIsFullscreen && mExpandedDesktopStyle != 3) {
