...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    boolean mBottomBar = false;
    private int mLongVolpAction;
    private int mLongVolmAction;
    ...
    Runnable mVolumeUpLongPress = new Runnable() {
        public void run() {
            ...
            mIsLongPress = true;
            int keycode=translateActionToKeycode(mLongVolpAction);

            handleVolumeEvents(keycode);
        };
    };
    ...
    Runnable mVolumeDownLongPress = new Runnable() {
        public void run() {
            // set the long press flag to true
            mIsLongPress = true;
            int keycode=translateActionToKeycode(mLongVolmAction);
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            mLongVolpAction = Settings.System.getInt(resolver,
                    Settings.System.LONG_VOLP_ACTION, 0);
            mLongVolmAction = Settings.System.getInt(resolver,
    ...
    private void handleVolumeEvents(int keycode)
    {
        if(!mScreenOn && isMusicActive() && mVolBtnMusicControls){
            sendMediaButtonEvent(KeyEvent.KEYCODE_MEDIA_NEXT);
        }
        if(mScreenOn && mLongVolpAction != 0){
            if(keycode==VOLUME_ACTION_LONG_HOME){
                mHomeLongPress.run();
            }else if(keycode==KeyEvent.KEYCODE_MEDIA_NEXT || keycode==KeyEvent.KEYCODE_MEDIA_PREVIOUS || keycode==KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE){
    ...
    public int interceptKeyBeforeQueueing(long whenNanos, int action, int flags,
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_UP: {
                if((mVolBtnMusicControls || mLongVolmAction!=0 || mLongVolpAction!=0) && !down)
                {
                ...
                if (down) {
                    ...
                    if(mVolBtnMusicControls || mLongVolmAction!=0 || mLongVolpAction!=0) {
