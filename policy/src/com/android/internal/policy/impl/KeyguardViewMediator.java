...
public class KeyguardViewMediator implements KeyguardViewCallback,
    ...
    public void onScreenTurnedOff(int why) {
        synchronized (this) {
            ...
            if (mExitSecureCallback != null) {
                ...
                resetStateLocked();
                if(mUpdateMonitor.isLockPatternDelayed()) {
                    startScreenDelay(why);
                }
            } else if (why == WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT
                    || why == WindowManagerPolicy.OFF_BECAUSE_OF_USER) {
                if(!startScreenDelay(why))
                    doKeyguard();
    ...
    /**
     * Checks if there are screen-delays configured and starts timers therefore
     * @param turnOffReason Why was the screen turned off
     * @return Did we add an alarm for the lockscreen
     */
    private boolean startScreenDelay(int turnOffReason) {
        final ContentResolver cr = mContext.getContentResolver();

        // retrieve the correct timeout setting depending on why
        // the screen went off
        int timeoutDelay = (turnOffReason == WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT ? Settings.System
        ...
        int securityTimeoutDelay = (turnOffReason == WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT ? Settings.System
        ...
        if(securityTimeoutDelay > 0) {
            ...
            delayedScreenLockOn(securityTimeoutDelay, LockScreenType.UNLOCK);
        }

        // if there is a delay, turn on the screen lock after the delay
        if(timeoutDelay > 0) {
           delayedScreenLockOn(timeoutDelay, LockScreenType.LOCK);
           return true;
        }
        return false;
    }
     ...
     *
     * @param delay The delay after which the lockscreen is activated
     * @param type Which lockscreen should be activated after the delay
     */
    private void delayedScreenLockOn(int delay, LockScreenType type) {
        final ContentResolver cr = mContext.getContentResolver();
        ...
        Intent intent = new Intent((type == LockScreenType.UNLOCK) ? DELAYED_SECURITY_ACTION
        ...
        if (DEBUG)
            Log.d(TAG, "setting alarm to turn on " + type.name() + "-Screen, seq = "
