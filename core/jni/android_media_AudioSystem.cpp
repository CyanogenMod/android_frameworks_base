...
static void
android_media_AudioSystem_effect_session_callback(int event, audio_stream_type_t stream, audio_unique_id_t sessionId)
{
    JNIEnv *env = AndroidRuntime::getJNIEnv();
    if (env == NULL) {
        return;
...
"(Landroid/media/AudioHandle;[Landroid/media/AudioPortConfig;[Landroid/media/AudioPortConfig;)V");
    ...
    gEffectSessionEventHandlerMethods.postEffectSessionEventFromNative =
            GetStaticMethodIDOrDie(env, env->FindClass(kClassPathName),
                    "effectSessionCallbackFromNative", "(III)V");
