...
class VerbosePrinter:
    ...
    def _start_ticker(self):
        self.ticker = VerbosePrinter.Ticker(self.cond_var)
        self.ticker.start()
    ...
    def begin(self, type, name):
        sys.stdout.write(self._format_begin(type, name))
        sys.stdout.flush()
        self._start_ticker()

    def end_pass(self, type, name):
        self._stop_ticker()
        sys.stdout.write('\b\b\b\b\b\b[ OK ]\n')
        sys.stdout.flush()

    def end_fail(self, type, name, msg):
        self._stop_ticker()
        sys.stdout.write('\r' + self.color_RED)
        sys.stdout.write(self._format_begin(type, name))
        sys.stdout.write('\b\b\b\b\b\b[FAIL]\n')
...
class IdmapScanTask:
    ...
    def execute(self):
        return _adb_shell('su system idmap --scan "%s" "%s" "%s" "%s" "%s"' % (self.overlay_dir, self.target_pkg_name, self.target_pkg, self.idmap_dir, self.symlink_dir))
    ...
    for line in stdout.split("\n"):
        ...
        if m:
            ...
            if c == 1:
                ...
                msg = "\n".join(current_stack)
                printer.end_fail(TASK_INSTRUMENTATION_TEST, current_test, msg)
            continue
...
if __name__ == '__main__':
    ...
    tasks.append(PushTask('/system/app/OverlayTest.odex', '/system/app/OverlayTest.odex'))
    ...
    if opts.test_idmap:
        ...
        # idmap symlinks
        # re-use setup from idmap --scan
        symlink_a = symlinks + '/system@framework@framework-res.apk/0000'
        symlink_b = symlinks + '/system@framework@framework-res.apk/0001'
        tasks.append(FileExistsTest(symlink_a))
        tasks.append(FileExistsTest(symlink_b))
        tasks.append(GrepIdmapTest(symlink_a, 'string/rro_test_fruit', 1))
        tasks.append(GrepIdmapTest(symlink_b, 'bool/rro_test_usually_false', 1))
