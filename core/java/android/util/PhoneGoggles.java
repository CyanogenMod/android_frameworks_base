...
public class PhoneGoggles {
    ...
    public static void processCommunication(Context context,
            ...
            Runnable onCancel, String applicationDescriptionSetting,
            int title,  int titleUnlocked, int dialogContent,
            int communicationUnauthorized, int communicate, int cancel) {

        final ContentResolver contentResolver = context.getContentResolver();
        final boolean inPhoneGoggles = PhoneGoggles.inPhoneGoggles(
                contentResolver, applicationDescriptionSetting);
        ...
        if (inPhoneGoggles) {
            ...
            if (areFiltered) {
                int confirmationGogglesMode = Settings.System.getInt(
                        contentResolver,
                ...
                switch (confirmationGogglesMode) {
                    case CONFIRMATION_MODE_PROMPT:
                        showSimpleDialog(context, onCommunicate, onCancel,
                                title, dialogContent, communicate, cancel);
                        return;
                    ...
                    case CONFIRMATION_MODE_MATHS:
                        showPhoneGogglesDialog(context, onCommunicate, onCancel,
                                title, titleUnlocked, communicate, cancel);
                        return;
                }
            }
        }
    ...
    private static boolean inPhoneGoggles(ContentResolver contentResolver,
            String applicationEnabledSetting) {
        boolean gogglesEnabled = Settings.System.getInt(contentResolver,
                Settings.System.PHONE_GOGGLES_ENABLED, 0) != 0;

        // Communications are enabled by default
        boolean communicationGogglesEnabled = Settings.System.getInt(
                contentResolver, applicationEnabledSetting, 1) != 0;
        int gogglesStart = Settings.System.getInt(contentResolver,
                Settings.System.PHONE_GOGGLES_START, 1320);
        int gogglesEnd = Settings.System.getInt(contentResolver,
                Settings.System.PHONE_GOGGLES_END, 300);

        if (gogglesEnabled && communicationGogglesEnabled
                && (gogglesStart != gogglesEnd)) {
