...
public class ResourcesManager {
    ...
    private boolean attachThemeAssets(AssetManager assets, CustomTheme theme) {
        ...
        try {
            piTheme = getPackageManager().getPackageInfo(theme.getThemePackageNameForApp(basePackageName), 0, UserHandle.myUserId());
            piTarget = getPackageManager().getPackageInfo(basePackageName, 0, UserHandle.myUserId());
            ...
            if (piTarget == null && resourcePackageName != null) {
                piTarget = getPackageManager().getPackageInfo(resourcePackageName, 0, UserHandle.myUserId());
            }
        ...
        if (!piTarget.isThemeApk && piTheme.mOverlayTargets.contains(basePackageName)) {
            ...
            String prefixPath = piTheme.isLegacyThemeApk ? "" : ThemeUtils.getOverlayPathToTarget(basePackageName);
