...
public class ResourcesManager {
    ...
    private void setActivityIcons(Resources r) {
        ...
        try {
            Bitmap[] iconBack = pm.getIconBackBitmap();
            BitmapDrawable[] iconBackDrawables = null;
            if (iconBack != null && iconBack.length > 0) {
                iconBackDrawables = new BitmapDrawable[iconBack.length];
                for (int i = 0; i < iconBack.length; i++) {
                    iconBackDrawables[i] = new BitmapDrawable(r, iconBack[i]);
                }
            }
            Bitmap iconMask = pm.getIconMaskBitmap();
            Bitmap iconUpon = pm.getIconUponBitmap();
            float iconScale = pm.getIconScale();
            int iconSize = pm.getIconSize();
            r.setCustomIcons(iconBackDrawables,
                    iconMask != null ? new BitmapDrawable(r, iconMask) : null,
                    iconUpon != null ? new BitmapDrawable(r, iconUpon) : null, iconScale, iconSize);
            ...
            if (r != null) {
                ...
                if (themeChanged) {
                    ...
                    if (am.hasThemeSupport()) {
                        ...
                        r.setCustomIcons(null, null, null, 1.0f, 0);
