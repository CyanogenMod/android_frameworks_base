...
public class Activity extends ContextThemeWrapper
    ...
    final void attach(Context context, ActivityThread aThread,
        ...
        if (voiceInteractor != null) {
            ...
            // burn it all down.
            IVoiceInteractionManagerService mVoiceInteractionManagerService
                    = IVoiceInteractionManagerService.Stub.asInterface(
                    ServiceManager.getService(Context.VOICE_INTERACTION_MANAGER_SERVICE));
            try {
                Bundle b = new Bundle();
                int flag = VoiceInteractionSession.SHOW_WITH_ASSIST;
                VoiceCallback vc = new VoiceCallback();
                voiceInteractor = mVoiceInteractionManagerService.requestCurrentSession(
                        b, flag, vc, getActivityToken());
            } catch (RemoteException e) {
                e.printStackTrace();
            }
            if (voiceInteractor != null) {
                Log.v("BIRD", "voice interactor is not null");
                mVoiceInteractor = new VoiceInteractor(voiceInteractor, this, this,
                        Looper.myLooper());
            } else {
                Log.v("BIRD", "sad");
