...
oneway interface IBackupAgent {
    ...
    /**
     * Perform a backup for all the files specified by domainTokens parameter. The output file is presumed
     * to be a socket or other non-seekable, write-only data sink.  When this method is
     * called, the app should write all of its files to the output.
     *
     * @param data Write-only file to receive the backed-up file content stream.
     ...
     *        contents.
     *
     * @param domainTokens File paths of the files (represented as domains) that need to be backed up.
     *
     * @param excludeFilesRegex Files to exclude - specified as a regex.
     *
     * @param callbackBinder Binder on which to indicate operation completion,
     *        passed here as a convenience to the agent.
     */
