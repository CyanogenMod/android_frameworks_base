...
public class Profile implements Parcelable {
    ...
    /** @hide */
    public Profile() {
    	// Generate new profile, with a random UUID and a template name.
    ...
    public void readFromParcel(Parcel in) {
        ...
        mStatusBarIndicator = in.readInt() == 1 ? true : false;
    ...
    public void setUuid(String newUuid) {
    	this.mUuid = UUID.fromString(newUuid);
    }
    ...
    public void getXmlString(StringBuilder builder) {
        builder.append("<profile name=\"" + TextUtils.htmlEncode(getName()) + "\" uuid=\"" + TextUtils.htmlEncode(getUuid().toString()) + "\" statusbar=\"" + TextUtils.htmlEncode((getStatusBarIndicator() ? "yes" : "no")) + "\">\n");
    ...
    public static Profile fromXml(XmlPullParser xpp, Context context)
        ...
        try {
        	profileUuid = UUID.fromString(getAttrResString(xpp, context, "uuid"));
        ...
        try {
        	profile.setStatusBarIndicator(getAttrResString(xpp, context, "statusbar") == "yes" ? true : false);
        } catch (NullPointerException e) {
        	Log.w(TAG, "Null Pointer - StatusBar Visible flag not found for " + profileName + ".  Assuming FALSE.");
        	profile.setStatusBarIndicator(false);
        }
        int event = xpp.next();
