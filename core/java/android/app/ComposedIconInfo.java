...
public class ComposedIconInfo implements Parcelable {
    ...
    public BitmapDrawable[] iconBack;
    ...
    private ComposedIconInfo(Parcel source) {
        ...
        if (backCount > 0) {
            iconBack = new BitmapDrawable[backCount];
            for (int i = 0; i < backCount; i++) {
                iconBack[i] = new BitmapDrawable((Bitmap) source.readParcelable(bmpClassLoader));
            }
        }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(iconBack != null ? iconBack.length : 0);
        if (iconBack != null) {
            for (BitmapDrawable d : iconBack) {
                dest.writeParcelable(d.getBitmap(), flags);
            }
        }
