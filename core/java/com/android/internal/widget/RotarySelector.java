...
public class RotarySelector extends View {
    ...
    public RotarySelector(Context context, AttributeSet attrs) {
         ...
         * or decreasing the virtual size of their screen by altering
         * the  dpi in the build.prop file. this is often done especially
         * for hd phones. keep in mind changing build.prop and density
         * isnt officially supported, but this should do for most cases
         */
        if(displayMetrics.densityDpi == DisplayMetrics.DENSITY_HIGH && densityDpi != 240)
            mDensityScaleFactor=(float)(240.0 / densityDpi);
        if(displayMetrics.densityDpi == DisplayMetrics.DENSITY_MEDIUM && densityDpi != 160)
