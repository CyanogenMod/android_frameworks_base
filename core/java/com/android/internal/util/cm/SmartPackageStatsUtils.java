...
public class SmartPackageStatsUtils {
     ...
     */
    public static ArrayList<ComponentName> getRelevantComponentForRightNow(Context context,
            int limit) {
        ...
        ArrayList<ComponentName> intervalInfos = getRelevantComponentForInterval(context,
                new Date(currentHour), new Date(lookahead), limit);

        for (ComponentName info : intervalInfos) {
            System.out.println("Relevant app " + info.toString());
        ...
        if (infos.size() < limit) {
            ArrayList<ComponentName> launchCountInfos = getMostLaunchedComponents(context, limit
                    - infos.size());
            for (ComponentName info : launchCountInfos) {
                System.out.println("Most launched " + info.toString());
                if (!infos.contains(info)) {
                    infos.add(info);
                }
            }
     ...
     */
    public static ArrayList<ComponentName> getRelevantComponentForInterval(Context context,
        ...
        if (c != null) {
            if (DEBUG) Log.d(TAG, "SmartPackageStatsUtils query successful , "
                    + c.getCount() + " matches");
            while (c.moveToNext()) {
                ComponentName info = getComponentNameForPackageAndComponent(context,
                        c.getString(0),
                        c.getString(1));
    ...
    private static void populateListFromLimit(Context context, ArrayList<ComponentName> infos,
        ...
        if (c != null) {
            if (DEBUG) Log.d(TAG, "SmartPackageStatsUtils query successful , "
                    + c.getCount() + " matches");
            while (c.moveToNext()) {
                if (DEBUG) Log.d(TAG, "SmartPackageStatsUtils query cursor " + DatabaseUtils.dumpCurrentRowToString(c));
                ComponentName info = getComponentNameForPackageAndComponent(context,
                        c.getString(0),
                        c.getString(1));
                if (info != null && !infos.contains(info)) {
    ...
    private static ComponentName getComponentNameForPackageAndComponent(Context context,
