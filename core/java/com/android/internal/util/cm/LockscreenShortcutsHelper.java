...
public class LockscreenShortcutsHelper {
    ...
    private void fetchTargets() {
        ...
        if (mTargetActivities.isEmpty()) {
            mTargetActivities.add(NavigationRingConstants.ACTION_NONE);
            mTargetActivities.add(NavigationRingConstants.ACTION_NONE);
        }
    }
    ...
    public List<TargetInfo> getDrawablesForTargets() {
        ...
        for (int i = 0; i < Shortcuts.values().length; i++) {
            ...
            if (TextUtils.isEmpty(activity) || activity.equals(NavigationRingConstants.ACTION_NONE)) {
                drawable = getDrawableFromSystemUI(i == 0 ? PHONE_DEFAULT_ICON : CAMERA_DEFAULT_ICON);
                filerToSet = null;
            } else {
    ...
    public boolean isTargetCustom(Shortcuts shortcut) {
        return mTargetActivities != null &&
                ...
                !mTargetActivities.get(shortcut.index).equals(
                        NavigationRingConstants.ACTION_NONE);
    }
