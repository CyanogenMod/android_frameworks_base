...
public class ConfigSplitHelper {
    ...
    public static ArrayList<ButtonConfig> getButtonsConfigValues(Context context, String config,
        ...
        for (String configValue : config.split("\\" + NavigationRingConstants.ACTION_DELIMITER)) {
            counter++;
            if (counter == 1) {
    ...
    public static String setButtonsConfig(
        ...
        for (int i = 0; i < buttonsConfig.size(); i++) {
            if (i != 0) {
                finalConfig += NavigationRingConstants.ACTION_DELIMITER;
            }
            buttonConfig = buttonsConfig.get(i);
            finalConfig += buttonConfig.getClickAction() + NavigationRingConstants.ACTION_DELIMITER;
            if (!isShortcut) {
                finalConfig += buttonConfig.getLongpressAction()
                    + NavigationRingConstants.ACTION_DELIMITER;
            }
