...
public class AudioSystem
{
   ...
   /**
    ...
    {
        void onAudioEffectSessionCreatedForStream(int stream, int sessionId);
    }

    //keep in sync with include/media/AudioPolicy.h
    private final static int AUDIO_EFFECT_SESSION_CREATED = 10;
    ...
    private static void effectSessionCallbackFromNative(int event, int stream, int sessionId)
    {
        ...
        if (cb != null) {
            switch(event) {
                case AUDIO_EFFECT_SESSION_CREATED:
                    cb.onAudioEffectSessionCreatedForStream(stream, sessionId);
                    break;
                default:
